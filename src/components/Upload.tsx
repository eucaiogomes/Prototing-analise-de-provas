...
const etapas = [
  'Enviando o arquivo',
  'Analisando tela, câmera e áudio',
...
export function Upload({
  ...
  totalEventos, paraRevisar, analises, erro, onIniciar, onEscolher, onReiniciar,
}: Props) {
  const input = useRef<HTMLInputElement>(null);
  const [sobre, setSobre] = useState(false);

  function receber(f: File | undefined) {
    if (f) onIniciar({ nome: f.name, tamanho: f.size });
  }

  // A varredura só corre durante a etapa de análise (15%–85% do total).
  const varredura = Math.min(1, Math.max(0, (progresso - 15) / 70));
  const etapa = progresso < 15 ? 0 : progresso < 85 ? 1 : 2;
            ...
            <div className="mt-6 border-t border-rule pt-4">
              <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
                <p className="text-[13.5px] text-ink" aria-live="polite">
                  <span className="text-mute">Etapa {etapa + 1} de 3</span>{' '}
                  <span className="mx-1 text-rule">/</span> {etapas[etapa]}
                </p>
                <p
        ...
        {!analisando && (
          <>
            <p className="mt-3 text-[12.5px] leading-relaxed text-mute">
              O envio ainda não dispara a análise: ela roda fora do navegador, por
              linha de comando. Enviar um arquivo abre a análise mais recente.
            </p>
